...
// XP, so we are loading it up dynamically to be on the safe side.
// If we can't find it, there are other options, but they are prone
// to their own set of issues.
//
...
void CrashRecoveryContext::Enable() {
  ...
  typedef PVOID (WINAPI *fpAddVectoredExceptionHandler)(ULONG, 
                         LONG (NTAPI *)(struct _EXCEPTION_POINTERS *));
  static fpAddVectoredExceptionHandler AddVectoredExceptionHandler;
  static bool sAttemptedPreviously;

  // Only try to look the function up once -- no sense in performing the
  // work countless times if the function doesn't exist.
  if (!sAttemptedPreviously) {
    sAttemptedPreviously = true;

    HMODULE hLib = ::LoadLibrary(TEXT("Kernel32.dll"));
    if (hLib) {
      AddVectoredExceptionHandler = (fpAddVectoredExceptionHandler) 
                         ::GetProcAddress(hLib, "AddVectoredExceptionHandler");
      ::FreeLibrary(hLib);
    }
  }
...
void CrashRecoveryContext::Disable() {
  ...
  if (currentHandle) {
    typedef PVOID (WINAPI *fpRemoveVectoredExceptionHandler)(PVOID);
    static fpRemoveVectoredExceptionHandler RemoveVectoredExceptionHandler;
    static bool sAttemptedPreviously;
    ...
    if (!sAttemptedPreviously) {
      ...
      HMODULE hLib = ::LoadLibrary(TEXT("Kernel32.dll"));
      if (hLib) {
        RemoveVectoredExceptionHandler = (fpRemoveVectoredExceptionHandler)
                      ::GetProcAddress(hLib, "RemoveVectoredExceptionHandler");
        ::FreeLibrary(hLib);
      }
    }
